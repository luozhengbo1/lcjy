    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                _self.gearArea.innerHTML = '<div class="area_ctrl slnameeInUp">' +
            ...
            function gearTouchStart(e) {
                ...
                clearInterval(target["int_" + target.name]);
                target["old_" + target.name] = e.targetTouches[0].screenY;
                target["o_t_" + target.name] = (new Date()).getTime();
                var top = target.getAttribute('top');
                if (top) {
                    target["o_d_" + target.name] = parseFloat(top.replace(/em/g, ""));
                } else {
                    target["o_d_" + target.name] = 0;
                }
            ...
            function gearTouchMove(e) {
                ...
                target["new_" + target.name] = e.targetTouches[0].screenY;
                target["n_t_" + target.name] = (new Date()).getTime();
                var f = (target["new_" + target.name] - target["old_" + target.name]) * 30 / window.innerHeight;
                target["pos_" + target.name] = target["o_d_" + target.name] + f;
                target.style["-webkit-transform"] = 'translate3d(0,' + target["pos_" + target.name] + 'em,0)';
                target.setAttribute('top', target["pos_" + target.name] + 'em');
            ...
            function gearTouchEnd(e) {
                ...
                var flag = (target["new_" + target.name] - target["old_" + target.name]) / (target["n_t_" + target.name] - target["o_t_" + target.name]);
                if (Math.abs(flag) <= 0.2) {
                    target["spd_" + target.name] = (flag < 0 ? -0.08 : 0.08);
                } else {
                    if (Math.abs(flag) <= 0.5) {
                        target["spd_" + target.name] = (flag < 0 ? -0.16 : 0.16);
                    } else {
                        target["spd_" + target.name] = flag / 2;
                    }
                }
                if (!target["pos_" + target.name]) {
                    target["pos_" + target.name] = 0;
                }
                rollGear(target);
            }
            //缓动效果
            function rollGear(target) {
                ...
                clearInterval(target["int_" + target.name]);
                target["int_" + target.name] = setInterval(function() {
                    var pos = target["pos_" + target.name];
                    var speed = target["spd_" + target.name] * Math.exp(-0.03 * d);
                    pos += speed;
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                var name = item[gearVal][this.keys['name']];
                var childData;
                switch (_self.type) {
                    case 1:
                ...
                var itemStr = "";
                for (var i = 0; i < l; i++) {
                    itemStr += "<div class='tooth'  ref='" + item[i][this.keys['name']] + "'>" + item[i][this.keys['name']] + "</div>";
                }
